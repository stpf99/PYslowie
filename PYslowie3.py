...
class ProverbsSearchApp(QWidget):
    def __init__(self):
        ...
        # Model GPT-2
        self.gpt2_tokenizer = AutoTokenizer.from_pretrained("dkleczek/bert-base-polish-uncased-v1")
        self.gpt2_model = AutoModelForCausalLM.from_pretrained("dkleczek/bert-base-polish-uncased-v1")
    ...
    def gpt2_generate(self, selected_proverbs):
        ...
        generated_output = self.gpt2_model.generate(
            input_ids,
            do_sample=True,
            max_length=200,  # Ustaw maksymalną długość tekstu na 30 tokenów
            num_return_sequences=1,  # Generuj tylko jedno przysłowie
            temperature=1.0,  # Zmniejsz temperaturę dla bardziej zdyscyplinowanego tekstu
            no_repeat_ngram_size=2  # Unikaj powtarzających się fraz
        )
